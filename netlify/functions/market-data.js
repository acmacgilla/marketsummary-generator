...
// ... (All other code like API keys, formatPct, and fetchNewsHeadlines remains the same)
...
async function fetchNewsHeadlines() {
    // ... (News headline code is unchanged)
    ...
    } else { console.error("World News API failed:", results[0].reason?.response?.data || "Unknown Error"); }
    ...
    } else { console.error("NewsAPI failed:", results[1].reason?.response?.data || "Unknown Error"); }
    if (headlines.length === 0) { return ["- Could not fetch headlines from any source."]; }
...
async function fetchMarketData() {
    ...
    // ... (The rest of the function with all the await getSymbolData calls and formatting is unchanged)
